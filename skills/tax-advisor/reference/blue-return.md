...
> **免責**: 以下はR8大綱（R7.12閣議決定）に基づく見通しであり、法案成立前の情報。

| 条件 | 控除額 |
|------|--------|
| 優良な電子帳簿保存 + e-Tax + 期限内申告 | **75万円**（新設） |
| 複式簿記 + e-Tax + 期限内申告 | **65万円** |
| 複式簿記 + 書面提出 + 期限内申告 | **65万円**（55万円は廃止） |
| 簡易簿記 or 期限後申告 | **10万円**（事業収入1,000万円超は適用不可） |

主な変更点:
- **75万円控除を新設**: 電子帳簿保存法の「優良な電子帳簿」の要件を満たす場合
- **55万円控除を廃止**: 書面提出でも65万円控除に統一
- **10万円控除に収入制限**: 事業収入（不動産収入含む）が1,000万円を超える場合は10万円控除の適用不可
