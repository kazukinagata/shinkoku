...
2. **デュアルエージェント OCR 検証で画像を読み取る:**
   ...
   **結果照合:** 両エージェントの結果から `total_amount`, `date`, `vendor` を比較する
...
3. **デュアルエージェント OCR 検証で全ファイルを読み取る:**
   ...
   **結果照合:** ファイル単位で両エージェントの `total_amount`, `date`, `vendor` を比較する
...
2. **結果照合:** 両エージェントの結果から `total_amount`, `tax_amount`, `date`, `vendor` を比較する
