...
株式取引・FX取引・仮想通貨の判定フローチャートに従って申告要否を判定する。

詳細なフローチャートは `references/separate-tax-flowchart.md` を参照。

判定のポイント:
- 特定口座（源泉徴収あり）のみの場合、損益通算や繰越損失がなければ申告不要
- FXは利益が出ている場合は確定申告が必要（分離課税）
- 仮想通貨は雑所得（総合課税）として20万円超で申告必要
...
詳細なフローチャートや判定ロジックは以下を参照:
- **`references/separate-tax-flowchart.md`** — 株式・FX・仮想通貨の分離課税判定フローチャート
