...
```
人的控除差調整額の例:
  基礎控除: 所得税58万円（R7は58万〜95万、R8は62万〜104万） - 住民税43万円 → 調整控除上の差額は一律5万円で計算
...
- **住民税の基礎控除は43万円で据え置き**（令和9年度分も変更なし）。上限額を決める住民税所得割額は変わらないため、給与所得控除の変更を受けない層では上限額もほぼ変わらない
- 所得税の課税所得から住民税所得割を逆算する簡易推定は、基礎控除差（最大61万円 → 所得割6.1万円）の分だけ上限を過少に見積もる。`shinkoku tax calc-furusato-limit` の詳細推定（住民税側の控除で計算）を使う
