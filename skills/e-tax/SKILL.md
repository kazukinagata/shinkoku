...
xtx に含まれる帳票を確認する。対応帳票の一覧は `/capabilities` スキルを参照。

| 帳票コード | 帳票名 | 条件 |
|-----------|--------|------|
| KOA020-1 | 申告書B 第一表 | 常に含まれる |
| KOA020-2 | 申告書B 第二表 | 常に含まれる |
| KOA210 | 青色申告決算書（PL + BS） | 事業所得ありの場合 |
| KOB060 | 所得の内訳書 | 常に含まれる |
| KOB130 | 住宅借入金等特別控除額の計算明細書 | 住宅ローン控除ありの場合 |
| SHA010 | 消費税及び地方消費税申告書 | 課税事業者の場合（別xtxファイル） |
...
ユーザーに省略可能な書類を案内し、原本の5年間保存義務を伝える。
