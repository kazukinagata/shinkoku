...
1. **所得税の確定申告書**: 第一表（収入・所得・控除・税額）、第二表（内訳・明細）
2. **青色申告決算書**: 損益計算書・貸借対照表の整合性（該当者のみ）
3. **消費税申告書**: 課税売上・納付税額の検証（該当者のみ）
4. **第三表（分離課税）**: 株式・FXの所得・税額の検証（該当者のみ）
5. **第四表（損失申告）**: 繰越損失の記載内容の検証（該当者のみ）
6. **減価償却明細書**: 固定資産の償却計算の検証（該当者のみ）
7. **添付書類**: 源泉徴収票、控除証明書、特定口座報告書等

### 第三表・第四表の提出要否チェック

- **第三表**: 株式譲渡所得または FX 雑所得がある場合に必須。`doc_generate_schedule_3` で生成済みか確認する
- **第四表**: 繰越損失がある場合に必須。`doc_generate_schedule_4` で生成済みか確認する
- 未生成の場合は `/income-tax` スキルで分離課税の計算・PDF生成を実施するよう案内する

### ステップ1b: PDF出力の目視確認

生成済みPDFの視覚的な品質を確認するサブステップ。

1. `doc_preview_pdf` ツールで生成済みPDFをページ画像に変換する
2. 各ページの画像を Read ツールで確認する
3. 以下の視覚的チェックポイントを検証:

```
□ NTA公式用紙の背景が正しく表示されているか（テンプレートが存在する場合）
□ 数値が所定の欄に正しく配置されているか（枠からはみ出していないか）
□ 氏名・住所等のテキストが正しい位置にあるか
□ 桁別セルの数字が1桁ずつ正しくセルに入っているか
□ 文字の切れ・重なりがないか
□ Portrait/Landscape の向きが正しいか
```

4. 問題がある場合はユーザーに報告し、座標の微調整が必要な旨を案内する
5. テンプレートPDFが未配置（`templates/` ディレクトリが空）の場合はスキップ可能
...
詳細なチェック項目は以下を参照:
- **`references/submission-checklist.md`** — 第一表・第二表・決算書・消費税・分離課税・添付書類の全チェック項目
